...
The dependency graph is checked at the type level. Each module declares what it needs (a Secret, a Namespace, a database) and what it provides. When you compose modules into an environment, `fromModules` refuses to compile until every need has a provider. See [Dependency graph](/konfig.ts/docs/concepts/dependency-graph/).
