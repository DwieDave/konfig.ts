...
2. **Import the entry module.** Its default export must be an Effect: an `AppOfApps.fromModules({ ... })` or `Bundle.fromModules({ ... })` program. A missing default export, a non-Effect, or an import failure fails with `EnvLoadError { entry, cause }`.
