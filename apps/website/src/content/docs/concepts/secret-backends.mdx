...
- A failed `kubeseal` invocation, a non-YAML result (`KubesealParseError`), or a schema mismatch all surface as a `RenderError` with the message `... kubeseal failed` and the underlying error as `cause`.
