...
<Snippet file="infra/envs/prod.ts" startAt="export default AppOfApps.fromModules" title="infra/envs/prod.ts" />

The check in practice: if the `api` module needs the `ghcr-pull` secret (it yields `Dep.Secret("ghcr-pull")`) and none of the listed modules provides it, `fromModules` refuses to compile with `Missing provider for Secret "ghcr-pull"`. See [Dependency graph](/konfig.ts/docs/concepts/dependency-graph/).
