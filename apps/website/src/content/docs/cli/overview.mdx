...
`konfig --help` lists the subcommands, `konfig <command> --help` lists arguments and flags, and `konfig --version` prints the package version. The CLI depends on Effect 4 (release candidate) and `@effect/platform-node`. The package accepts any build in its supported rc range; see the [README requirements](https://github.com/DwieDave/konfig.ts/tree/main/packages/cli#requirements).
