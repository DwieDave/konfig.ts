...
//
// `secretEnv({ ref, key })` constrains `key` to the union K carried by
...
import { Secret, secretEnv } from "@konfig.ts/k8s";
...
// 4 · `secretEnv` accepts a declared key.
const _ok = secretEnv({ name: "DATABASE_URL", ref: dbCreds.ref, key: "url" });

// 5 · `secretEnv` rejects a typo on a typed ref.
const _typo = secretEnv({
...
// 6 · `secretEnv` rejects a key from a sibling secret.
const s3 = Secret.make({
	name: "s3-creds",
	namespace: "prod",
	stringData: { accessKey: "a", secretKey: "s" },
});
const _cross = secretEnv({
	name: "MISWIRED",
	ref: s3.ref,
...
const _anyKey = secretEnv({ name: "OPAQUE", ref: opaque, key: "anything" });
