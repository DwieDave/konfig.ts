...
import type { DefinedPod, Volume, VolumeNamesOf } from "@konfig.ts/k8s";
import {
	defineContainer,
	definePod,
	emptyDirVolume,
...
const cfg = emptyDirVolume({ name: "config" });
type _CfgN = Expect<Equal<typeof cfg, Volume<"config">>>;

// 2 · VolumeNamesOf<V> extracts the union of literal names.
type Vs = readonly [Volume<"config">, Volume<"data">];
...
const _typo = definePod({
	volumes: [emptyDirVolume({ name: "config" })],
	containers: [
		...
		defineContainer({
			...
			image: "x",
			ports: [port({ name: "http", containerPort: 8080 })],
			volumeMounts: [{ name: mountRef("cnofig"), mountPath: "/etc/conf" }],
		}),
	],
});
...
const _otherPod = definePod({
	volumes: [emptyDirVolume({ name: "logs" })],
	containers: [
		defineContainer({
			name: "log",
			image: "x",
			ports: [],
			volumeMounts: [{ name: mountRef("logs"), mountPath: "/var/log" }],
		}),
	],
});
