...
describe("serialize — key order (FR-2.1 / FR-2.2)", () => {
	...
	it("nested metadata (depth > 1) is alphabetical", () => {
		...
		const out = serialize(resource);
		// Inside spec.template.metadata: alphabetical => annotations, labels, name
...
describe("serialize — round-trip (FR-2.7, NFR-4.3)", () => {
	// `null` is intentionally excluded from the value arbitrary: the
	// serializer drops null-valued object keys to match nixidy's output
	// (helm `toYaml` of an empty value otherwise produces `key: null`
	// which nixidy strips). The round-trip property therefore only holds
	// for null-free trees; null handling is checked separately below.
...
describe("serialize — null stripping (helm parity)", () => {
	...
	it("preserves explicit null inside arrays (positional semantics)", () => {
		const out = serialize({ list: [1, null, 3] });
	...
	it("recursively strips nulls from nested objects", () => {
		const out = serialize({ a: { b: null, c: 1 }, d: null });
...
describe("serialize — line endings (FR-2.6)", () => {
	it("uses LF and ends with exactly one newline", () => {
		const out = serialize({ apiVersion: "v1", kind: "Pod", metadata: { name: "n" } });
