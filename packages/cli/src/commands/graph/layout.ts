...
const _nodeCenter = (ln: LaidNode): number => ln.boxStartX + Math.floor(ln.boxWidth / 2)
...
const _allocateMonotonic = (
  ...
  if (N === 0) return []
  const slots: number[] = ideals.map((x) => Math.max(slotMinX, Math.min(slotMaxX, x)))
  for (let i = 1; i < N; i++) {
...
const _computeNodeSlots = (
  ...
  const inOrdered = inEdges.slice().sort((a, b) => {
    const aParentSlot = slotsByNode.get(a.from)?.outgoingSlotX.get(laid.data.name)
    const bParentSlot = slotsByNode.get(b.from)?.outgoingSlotX.get(laid.data.name)
...
export const renderGraph = (input: RenderInput): string => {
  ...
  const scope = input.target !== undefined
  ...
  if (layout.canvasWidth > input.width) {
    const notice =
      `⚠ graph wider than terminal (${layout.canvasWidth} > ${input.width} cols); falling back to tree view.  Pass --width to override.`
