...
export const buildCommand = Command.make(
  ...
  (args) =>
    Effect.gen(function*() {
      ...
      const ctxOutDir = envOutDir({ cfg, envName: args.env, ctx, pathJoin: path.join })

      let cachedInputHash: string | undefined
      if (!args.noCache) {
        const inputHash = yield* computeInputHash({ cfg, envName: args.env, ctx })
        cachedInputHash = inputHash
        const entry = yield* readCacheEntry({ cfg, envName: args.env, ctx })
        if (
          entry !== undefined &&
