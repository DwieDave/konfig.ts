...
const parseHealthcheck = (line: string): Instruction => {
	...
	const partial: { -readonly [K in keyof Extract<HealthcheckIR, { _tag: "Cmd" }>]?: Extract<HealthcheckIR, { _tag: "Cmd" }>[K] } = { _tag: "Cmd", argv };
...
const healthArb: fc.Arbitrary<Instruction> = fc
	.tuple(argvArb, fc.option(duration, { nil: undefined }), fc.option(fc.integer({ min: 1, max: 5 }), { nil: undefined }))
